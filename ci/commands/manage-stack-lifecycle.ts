...
class SystemInitiativeClient {
  ...
  constructor() {
    ...
    if (!this.apiToken || !this.workspaceId) {
      throw new Error('Missing required environment variables: SI_API_TOKEN or SI_WORKSPACE_ID');
    }
  }
  ...
  async getActionLogs(changeSetId: string, funcRunId: string): Promise<any[]> {
    try {
      ...
      console.error(`❌ Failed to retrieve logs for func run ${funcRunId}: ${e}`);
      return [];
    }
  }

  async waitForMergeSuccess(changeSetId: string, timeoutSeconds = 300, pollInterval = 10): Promise<boolean> {
      ...
      if (actions.length === 0) {
        ...
        if (states.every((state: string) => state === 'Success')) {
          ...
          if (failedActions.length > 0) {
            console.log(`❌ ${failedActions.length} action(s) failed. Outputting logs:`);
            
            for (const action of failedActions) {
              console.log(`\n--- Logs for failed action: ${action.displayName || action.name || 'Unknown'} ---`);
              ...
              if (!funcRunId) {
                try {
                  ...
                  if (actionResponse.ok) {
                    ...
                    const actionDetail = actionsData.actions?.find((a: any) => a.id === action.id);
              ...
              if (funcRunId) {
                ...
                if (logs.length > 0) {
                  for (const log of logs) {
                    ...
                    if (stream === 'output' && (message.toLowerCase().includes('error') || message.includes('message'))) {
                      try {
  ...
  async getPublicIp(changeSetId: string, componentId: string, timeoutSeconds = 60, pollInterval = 3): Promise<string> {
  ...
  async forceApplyWithRetry(changeSetId: string, timeoutSeconds = 120, retryInterval = 5): Promise<any> {
    const startTime = Date.now();
    
    while (Date.now() - startTime < timeoutSeconds * 1000) {
      try {
        const forceApplyUrl = `${this.apiUrl}/v1/w/${this.workspaceId}/change-sets/${changeSetId}/force_apply`;
        ...
        if (!response.ok) {
          if (response.status === 428) {
            ...
            console.log(`⏳ DVU Roots still present. Retrying in ${retryInterval}s... (${(remaining / 1000).toFixed(1)}s remaining)`);
    ...
    const forceApplyTimeoutMsg = `Force apply timeout: Change set apply failed after ${timeoutSeconds}s - DVUs still processing`;
  ...
  async createChangeSet(name: string): Promise<any> {
    try {
      const response = await this.fetch(`${this.apiUrl}/v1/w/${this.workspaceId}/change-sets`, {
        method: 'POST',
  ...
  async createComponent(changeSetId: string, schemaName: string, name: string, options?: any): Promise<any> {
  ...
  async getChangeSet(changeSetId: string): Promise<Response> {
    return await this.fetch(`${this.apiUrl}/v1/w/${this.workspaceId}/change-sets/${changeSetId}`);
  }
    ...
    try {
      ...
      const ec2Options = {
        attributes: {
          ...
          "/domain/UserData": {
            "$source": {
              "component": `tonys-chips-userdata`,
      ...
      if (!success) {
        try {
          ...
          await this.writeErrorToFile("Deployment actions failed - check logs for details");
        }
      ...
      try {
        const publicIp = await this.getPublicIp(baseChangeSetId, ec2ComponentId, 120, 5);
