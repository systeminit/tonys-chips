...
/**
 * Summarize report content for GitHub issue (when full report is too long)
 */
function summarizeReportForIssue(
  policyTitle: string,
  ...
  if (result.failingComponents.length > 0) {
    summary += `**Total Failing Components**: ${result.failingComponents.length}\n\n`;

    // Show first 10 failing components
    const componentsToShow = result.failingComponents.slice(0, 10);
    summary += `### Failing Components (showing ${componentsToShow.length} of ${result.failingComponents.length}):\n\n`;
    summary += '| component | description |\n';
    summary += '| -- | -- |\n';

    for (const component of componentsToShow) {
      const componentLink = `[${component.schema} ${component.name}](${component.deeplink})`;
      summary += `|${componentLink}|${component.description}|\n`;
    }

    if (result.failingComponents.length > 10) {
      summary += `\n_...and ${result.failingComponents.length - 10} more. See full report in artifacts._\n`;
    }
    summary += '\n';
  }
...
/**
 * Generate GitHub Actions artifact link
 */
function getArtifactLink(): string | null {
...
async function postPolicyResultToIssue(
  ...
  try {
    ...
    // Get artifact link
    const artifactLink = getArtifactLink();

    // Build issue body header
    let issueBodyContent = `<!-- policy-check:${policyIdentifier} -->
# ${policyTitle}

**Status**: ${result.result === 'Pass' ? '✅ **PASS**' : '❌ **FAIL**'}
**Policy File**: \`${policyFilename}\`
**Workspace**: [View in System Initiative](https://app.systeminit.com/n/${workspaceId})
**Date**: ${new Date().toISOString()}
`;

    // Add artifact link if available
    if (artifactLink) {
      issueBodyContent += `**Full Report**: [📄 View in Artifacts](${artifactLink})\n`;
    }

    issueBodyContent += '\n---\n\n';

    // Check if full report would exceed GitHub's limit (65536 chars)
    // Use a buffer of 1000 chars for safety (header + footer)
    const MAX_BODY_LENGTH = 64500;
    const headerFooterLength = issueBodyContent.length + 200; // approximate footer length
    const availableSpace = MAX_BODY_LENGTH - headerFooterLength;

    let reportSection: string;
    let isTruncated = false;

    if (reportContent.length > availableSpace) {
      // Use summarized version
      reportSection = summarizeReportForIssue(policyTitle, result, policyText);
      isTruncated = true;
      console.log(`  ⚠️  Report truncated (${reportContent.length} chars → ${reportSection.length} chars)`);
    } else {
      reportSection = reportContent;
    }

    issueBodyContent += reportSection;
    issueBodyContent += '\n---\n\n';

    // Add truncation notice if needed
    if (isTruncated) {
      issueBodyContent += `> ⚠️ **Note**: This report was truncated due to size limits. `;
      if (artifactLink) {
        issueBodyContent += `[View the complete report in the workflow artifacts](${artifactLink}).\n\n`;
      } else {
        issueBodyContent += `View the complete report in the workflow artifacts.\n\n`;
      }
    }

    issueBodyContent += `<details>
...
This issue was automatically generated by the policy checker workflow. When the workflow runs again for this policy, this issue will be closed and a new one will be created with updated results.
    ...
    // Add policy output tags as labels
    labels.push(...outputTags);

    // Create new issue
    await client.createIssue(issueTitle, issueBodyContent, labels);
